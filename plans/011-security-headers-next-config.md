...
> **Executor instructions**: 3 段階（Stage 1: 非 CSP ヘッダ → Stage 2: CSP Report-Only 計測 →
> Stage 3: nonce ベース CSP 強制）で進める。**Stage をまたいで一気に実装しない** — 各 Stage の
...
- コミット: Stage ごとに 1 コミット。形式は `feat(security): <subject>`。
