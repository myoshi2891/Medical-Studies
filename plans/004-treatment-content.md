...
- **Depends on**: plans/001, plans/002（`/treatment/*` ルート）, plans/003（エビデンス/文献規約）
...
## 追加するコンテンツ（plans/002 の `/treatment/*` 準拠）

| ページ（slug） | 内容の骨子 | 優先度 |
|---|---|---|
| `acute-treatment-of-headache` | 急性期治療の考え方。薬効群: アセトアミノフェン / NSAIDs / トリプタン（作用機序・使い分けの一般論）/ ジタン（5-HT1F 作動）/ ゲパント（CGRP 受容体拮抗・急性期）。成層化治療の概念、早期服薬の原則、MOH 予防への注意 | P1 |
...
- 本ファイル（治療カテゴリ拡張の設計）
- （後続実装フェーズで）`Types-of-headache/md-files/Treatment/*.md` の新規執筆
...
`acute-treatment-of-headache` / `preventive-treatment` の Markdown を
`Types-of-headache/md-files/Treatment/` に作成。plans/003 のエビデンス表記・参考文献規約に従い、
...
nav-links.ts に `Treatment` ドロップダウンを追加（または既存の Treatment 項目を更新）。未完成ページは `disabled: true`。
...
- 既存 `app/treatment/acute-treatment-of-headache/page.test.tsx` および新規 `app/treatment/preventive-treatment/page.test.tsx` 等を、既存
...
- 検証: `bun run test -- treatment` で新規・既存テストが全 pass。
...
- [ ] plans/002 の `/treatment/*` slug をそのまま使用している
...
- 薬物療法は最も陳腐化が速い。新薬承認・適応拡大（PMDA/FDA/EMA）を plans/003 の随時監視で追い、
  `cgrp-targeted-therapies` と `acute-treatment-of-headache` を優先的に更新する。
