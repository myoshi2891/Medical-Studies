...
it("権利確認状態: HIT-6 / MSQ v2.1 は status を持つ（F1 管理対象）", () => {
  expect(REGISTRY.hit6.license.status).toBe("pending");
  expect(REGISTRY.hit6.license.officialUrl).toBe("https://www.qualitymetric.com/");
  expect(REGISTRY["msq-v2.1"].license.status).toBe("pending");
  expect(REGISTRY["msq-v2.1"].license.officialUrl).toBe("https://eprovide.mapi-trust.org/");
});
```
