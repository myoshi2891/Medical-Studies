...
```mermaid
flowchart TD
    CP["🔵 頚神経叢<br>（Cervical Plexus）<br>C1〜C4前枝から形成"]

    CP --> Shallow["浅枝（感覚枝）<br>4本の終末枝<br>皮膚・浅層の知覚を支配"]
    CP --> Deep["深枝（主に運動枝）<br>頸部深筋の運動支配<br>横隔神経（C3-C5）を含む"]

    Shallow --> B1["小後頭神経 Lesser Occipital<br>C2 → 後頭部外側・耳介後部"]
    Shallow --> B2["大耳介神経 Greater Auricular<br>C2/C3 → 耳介・耳下腺部"]
    Shallow --> B3["頸横神経 Transverse Cervical<br>C2/C3 → 頸部前面"]
    Shallow --> B4["鎖骨上神経 Supraclavicular<br>C3/C4 → 鎖骨上・肩峰部"]

    Deep --> D1["頸神経ワナ Ansa Cervicalis<br>C1-C3 → 舌骨下筋群"]
    Deep --> D2["横隔神経 Phrenic Nerve<br>C3-C5 → 横隔膜（運動）"]

    CP --> Block["💉 CPBの3タイプ"]
    Block --> SCB["浅頚神経叢ブロック<br>Superficial CPB<br>皮下注射/筋膜浅層"]
    Block --> ICB["中間頚神経叢ブロック<br>Intermediate CPB<br>深頸筋膜投資層と椎前筋膜の間"]
    Block --> DCB["深頚神経叢ブロック<br>Deep CPB<br>椎前筋膜深層/横突起傍"]
...
```mermaid
flowchart LR
    A["🔵 C2〜C4<br>脊髄神経前枝<br>（椎間孔内）"]
    B["椎前筋膜（Prevertebral Fascia）<br>深層を通って前方へ走行"]
    C["深頸筋膜の投資層<br>（Investing Layer of<br>Deep Cervical Fascia）<br>を貫通"]
    D["SCM後縁の中点<br>（甲状軟骨レベル）<br>←4本が同時に出現する「ホットスポット」"]
    E["🔴 皮下組織へ<br>各方向に扇状に走行<br>→ 前外側頸部・耳介・鎖骨上・後頭部外側"]
...
flowchart TB
    subgraph Surface["表層 ← 皮膚"]
        SKIN["🟤 皮膚<br>（Skin）"]
        SFC["浅頸筋膜<br>（Superficial Cervical Fascia）<br>広頸筋・浅静脈・リンパ節"]
    end

    subgraph DCF["深頸筋膜（Deep Cervical Fascia）"]
        IL["投資層<br>（Investing Layer / 深頸筋膜固有層）<br>SCM・僧帽筋を包む"]
        PCS["後頸部間隙<br>（Posterior Cervical Space: PCS）<br>← 中間CPBの標的空間"]
        PVF["椎前筋膜<br>（Prevertebral Fascia）<br>前・中斜角筋・頸長筋を包む"]
    end

    subgraph Deep["深層"]
        TP["頸椎横突起<br>（Transverse Processes C2-C4）<br>← 深CPBの針先目標"]
        VES["神経根・椎骨動脈"]
    end

    SKIN --> SFC --> IL --> PCS --> PVF --> TP

    SCB["💉 浅CPB<br>注入：皮下組織〜投資層の浅層<br>→ 主に感覚枝を遮断"]
    ICB["💉 中間CPB<br>注入：投資層と椎前筋膜の間（PCS）<br>→ 感覚枝＋一部深枝"]
    DCB["💉 深CPB<br>注入：椎前筋膜深層〜横突起傍<br>→ 感覚枝＋深枝（完全遮断）"]
...
```mermaid
flowchart TD
    START(["🩺 CPB候補患者<br>評価開始"])
    START --> Q1{"手術の侵襲深度は？"}

    Q1 -->|"皮膚・皮下組織のみ<br>（表在手術）"| SCB_ONLY["✅ 浅CPB単独<br>シンプル・安全"]
    Q1 -->|"頸部深層・甲状腺<br>（中等度侵襲）"| Q2

    Q2{"呼吸予備能は？"}
    Q2 -->|"正常"| Q3
    Q2 -->|"重篤なCOPD<br>対側横隔膜麻痺など"| CAUTION["⚠️ 深CPBは禁忌<br>浅CPBまたは中間CPBを<br>慎重に選択"]

    Q3{"CEAで神経学的<br>モニタリングが必要か？"}
    Q3 -->|"Yes（覚醒下）"| CEA_CHOICE["CEA: 浅CPB＋中間CPB（推奨）<br>または 浅CPB＋深CPB（従来法）"]
    Q3 -->|"No（甲状腺・頸部）"| THYROID["甲状腺：両側浅CPBまたは中間CPB<br>頸部郭清：浅＋中間CPBの組み合わせ"]

    SCB_ONLY --> PROCEED["🟢 手技実施へ<br>（禁忌確認後）"]
    CEA_CHOICE --> PROCEED
    THYROID --> PROCEED
    CAUTION --> PROCEED
...
```mermaid
flowchart LR
    A["乳様突起（Mastoid Process）<br>を触診・マーク"]
    B["鎖骨中点（Midclavicle）<br>をマーク"]
    C["SCM後縁を触診<br>乳様突起〜鎖骨中点<br>を結ぶ線上"]
    D["🎯 SCM後縁の中点<br>甲状軟骨切痕レベル<br>← 浅CPBの注射点"]
    E["皮膚消毒・局所浸潤麻酔<br>（任意）"]
    F["💉 注射：<br>25G針で皮下組織に扇状に注入<br>上方・後方・下方へ各2〜3 cm"]
...
flowchart TB
    subgraph US_Image["超音波画像（C4横断面）内側→外側"]
        direction LR
        IJV["内頸静脈<br>（IJV）<br>圧迫で虚脱する"]
        CCA["総頸動脈<br>（CCA）<br>拍動・圧迫で虚脱しない"]
        SCM_US["胸鎖乳突筋<br>（SCM）<br>比較的高エコー・三角形"]
        PLEXUS["🎯 頸神経叢<br>（C2-C4）<br>低エコーのハニカム状<br>SCM後縁深部〜外側"]
        LS["肩甲挙筋<br>（Levator Scapulae）<br>深層"]
        PVF_US["椎前筋膜<br>Prevertebral Fascia<br>高エコーの明瞭な線"]
    end

    IJV --- CCA --- SCM_US --- PLEXUS --- LS --- PVF_US

    TARGET["💉 浅CPBの注入目標<br>SCM後縁の外側〜深部の<br>皮下組織（投資層の外面）"]
    PLEXUS -.-> TARGET
...
```mermaid
flowchart LR
    A["プローブ設置<br>C4レベル・SCM後縁<br>横断位"]
    B["解剖構造同定<br>SCM・肩甲挙筋・<br>前斜角筋・椎前筋膜を確認"]
    C["投資層と椎前筋膜の間の<br>後頸部間隙（PCS）を同定<br>← 目標空間"]
    D["22G針を平面内アプローチで<br>外側→内側へ刺入<br>針先をPCSに進める"]
    E["逆吸引（Aspiration）<br>血液・CSFの逆流がないことを確認"]
    F["1〜2 mL注入で位置確認<br>（ハイドロダイセクション）<br>筋膜間に薬液が広がるか確認"]
    G["残量を分割投与<br>総量 10〜15 mL<br>注入時の抵抗に注意"]
...
```mermaid
flowchart TD
    P["患者体位<br>仰臥位・頭部反対側へ<br>10〜15°回旋<br>枕は使用しない（頸部伸展を保持）"]
    P --> L["ランドマーク同定<br>乳様突起・シャセニャック結節<br>→ ライン上にC2・C3・C4点をマーク"]
    L --> N["22G針を刺入<br>皮膚に対してわずかに尾側方向<br>（頭側方向への刺入は禁止→<br>神経孔への誤刺入リスク）<br>横突起に当たるまで進める（2〜3 cm）"]
    N --> A["逆吸引（必須）<br>血液（椎骨動脈）・CSF（くも膜下）の<br>逆流がないことを確認"]
    A -->|"陰性"| I["各点に3〜5 mL注入<br>総量 9〜15 mL（3点注射）<br>または単点注射（C4のみ: 10〜15 mL）"]
    A -->|"陽性"| R["⚠️ 即座に針を抜く<br>位置調整後に再試行<br>繰り返す場合は超音波ガイド下へ切り替え"]
...
flowchart TD
    ...
    COMP --> C1["🫁 横隔膜麻痺<br>（Phrenic Nerve Block）<br>頻度：深CPBで50〜80%<br>→ 中CPBでは低下、浅CPBではほぼなし<br>対側が正常であれば通常は問題なし"]
    COMP --> C2["🗣️ 反回喉頭神経ブロック<br>（Hoarseness）<br>頻度：まれ〜中程度<br>→ 嗄声・嚥下困難（一過性）"]
    COMP --> C3["😴 ホルネル症候群<br>（交感神経ブロック）<br>頻度：まれ<br>→ 縮瞳・眼瞼下垂・顔面無汗（一過性）"]
    COMP --> C4["🩸 椎骨動脈内注入<br>（Vertebral Artery Injection）<br>頻度：まれ<br>→ 少量でも全身痙攣・意識消失<br>（椎骨動脈は脳に直接繋がるため）"]
    COMP --> C5["💊 くも膜下腔誤注入<br>（Intrathecal Injection）<br>頻度：極めてまれ<br>→ 全脊髄麻酔・呼吸停止<br>【最重篤な合併症】"]
    COMP --> C6["🩸 頸部血腫<br>（Hematoma）<br>頻度：まれ<br>→ 気道圧迫（致死的になりえる）"]
...
```mermaid
flowchart TD
    SIGN["⚠️ LASTs初期症状<br>耳鳴り・金属味・口唇しびれ<br>視覚障害・不安・興奮"]
    SIGN --> STOP["1️⃣ 即座に注射を中止"]
    STOP --> CALL["2️⃣ 応援を呼ぶ<br>救急チームへ連絡"]
    CALL --> AIRWAY["3️⃣ 気道確保<br>100% O₂投与<br>必要時は気管挿管"]
    AIRWAY --> SEIZURE{"痙攣の有無"}
    SEIZURE -->|"あり"| BENZO["ベンゾジアゼピン静注<br>または<br>プロポフォール低用量"]
    SEIZURE -->|"なし"| MONITOR["継続モニタリング"]
    BENZO --> LIPID["4️⃣ 20%脂肪乳剤（Intralipid）<br>初回: 1.5 mL/kg iv（体重70 kgで約100 mL）<br>維持: 0.25 mL/kg/分 × 10分"]
    MONITOR --> LIPID
    LIPID --> CPR{"心停止？"}
    CPR -->|"あり"| ALS["心肺蘇生（CPR）開始<br>脂肪乳剤継続"]
    CPR -->|"なし"| OBSERVE["症状改善まで観察<br>30分以上"]
...
flowchart LR
    CEA["頸動脈内膜切除術<br>（CEA）"]
    CEA --> RA["局所麻酔を選択する場合<br>（覚醒下神経学的モニタリング目的）"]
    RA --> CHOICE["推奨：浅CPB＋中間CPB<br>（BJA Educ 2023）<br>or 浅CPB＋深CPB（従来法）"]
    CHOICE --> SUPPLE["術野への局所浸潤麻酔<br>（外科医による追加）<br>がしばしば必要"]
...
flowchart LR
    A["浅CPB実施<br>15〜30分後"]
    A --> B{"4枝の麻酔域内<br>感覚テスト<br>（ピン・冷覚）"}
    B -->|"麻酔域内：感覚消失"| C["✅ ブロック成功<br>手術開始可"]
    B -->|"不十分な感覚消失"| D{"追加注射の<br>必要性評価"}
    D -->|"追加可能"| E["追加2〜5 mL注入<br>（最大投与量の範囲内で）"]
    D -->|"最大量到達"| F["外科医による<br>局所浸潤麻酔を追加<br>or 全身麻酔への切り替え検討"]
