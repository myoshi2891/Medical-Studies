...
2. **大規模 HTML 作成時は 4 フェーズ分割コミット戦略の徹底。**
3. **違反検知時は即時報告。** サイクルを飛ばしたことに気づいた場合、独断で `git reset` 等を実行せず、直ちにユーザーへ報告し、承認を得たうえでリカバリ手順を実施すること。
</ai_agent_directive>
...
- **コミット（大規模 HTML 新規作成時）**:
  - 以下の 4 フェーズに分割してコミットする。コミットメッセージには必ず `Progress: N/M sections complete` を含める。
    - **Phase 1**: スケルトン（`<head>`, CSS, hero, disclaimer, sidebar, footer, script）
    - **Phase 2**: 前半セクション（セクション全体の約半分）
    - **Phase 3**: 残りのセクション
    - **Phase 4**: 修正・仕上げ（SRI ハッシュの確認、ナビゲーション、対比チェックなど）
