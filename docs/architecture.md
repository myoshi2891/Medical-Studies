...
| 3 | 各構造から md 教育ページへの誘導・やさしい言い換えの作り込み | 🔄 実装中（用語ツールチップ基盤＝`lib/glossary`＋`components/glossary/Term.tsx` を新設。読み仮名＋やさしい解説をツールチップ表示。`/anatomy` 凡例と主要ガイド（`app/headaches/` 4 ページ）に適用済み。残り画面は `.claude/skills/glossary-term-tooltip` の手順で展開） |
