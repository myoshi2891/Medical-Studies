...
- [ ] ファイル名に患者識別子・撮影日・施設名を含まない。
- [ ] ピクセル焼込みテキスト（氏名・ID・日付）が無い。
- [ ] 付随メタデータ（PNG tEXt 等）に PHI が無い。
- [ ] 顔貌の 3D 再構築が不可能な 2D スライスに限定する。
...
| Phase | 内容 |
|---|---|
| 0 | 雛形（ページシェル・manifest コア・遅延ビューア骨格・public 雛形） |
| 1 | 2D MRI スライスビューアの実画像投入（匿名化済み curate） |
| 2 | 3D 解剖モデル（オープンソース glTF）の投入・ホットスポット注釈 |
| 3 | 各構造から md 教育ページへの誘導・やさしい言い換えの作り込み |
| 4 | 仕上げ（パフォーマンス計測・アクセシビリティ・テスト拡充） |
