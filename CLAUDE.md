...
| `glossary-term-tooltip` | "用語ツールチップを追加", "やさしい解説を追加", "読み仮名を付ける", "用語集に追加" | web-next の専門用語に読み仮名＋高校生向けやさしい解説のツールチップを付与。用語集レジストリ（`lib/glossary`）に語を追記し、本文初出を `<Term>`（`components/glossary/Term.tsx`）でラップ。契約テスト非破壊手順を含む |
