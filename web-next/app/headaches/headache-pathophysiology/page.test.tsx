...
import HeadachePathophysiologyPage from "./page";
...
const TABLE_COUNT = 3;
const NAV_COUNT = 12;
const HERO_H1 = "頭痛の病態生理アップデート";

describe("HeadachePathophysiologyPage: 契約（忠実転記）", () => {
  it("頭痛カテゴリーのヒーローから本文へ進め、実際の件数を示す", () => {
    const { container } = render(<HeadachePathophysiologyPage />);
    expect(container.querySelector("header.hero .pth-breadcrumb")).toHaveTextContent("頭痛疾患");
    expect(container.querySelector("header.hero a[href='#s1']")).not.toBeNull();
    expect(
      Array.from(container.querySelectorAll(".pth-hero-stats dd"), (item) => item.textContent)
    ).toEqual([
 ...
 */
describe("HeadachePathophysiologyPage: 関連ページ導線", () => {
  const HREF = "/headaches/headache-pathophysiology";
