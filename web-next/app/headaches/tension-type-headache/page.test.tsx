...
describe("TensionTypeHeadachePage: 契約（忠実転記）", () => {
  it("頭痛カテゴリーのヒーローから本文へ進め、実際の件数を示す", () => {
    ...
    expect(container.querySelector("header.hero .tth-breadcrumb")).toHaveTextContent("頭痛疾患");
    expect(container.querySelector("header.hero a[href='#s1']")).not.toBeNull();
    expect(
      Array.from(container.querySelectorAll(".tth-hero-stats dd"), (item) => item.textContent)
    ).toEqual([
 ...
 */
describe("TensionTypeHeadachePage: 関連ページ導線", () => {
  const HREF = "/headaches/tension-type-headache";
