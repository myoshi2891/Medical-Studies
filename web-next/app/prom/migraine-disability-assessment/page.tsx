...
const MIDAS_MERMAID_THEME = {
  primaryColor: "#fadee4",
  primaryTextColor: "#6a1b2a",
  primaryBorderColor: "#b3204b",
  lineColor: "#546e7a",
...
export default function MigraineDisabilityAssessmentPage() {
  return (
    <div className="migraine-disability-assessment-accent">
      ...
      <div className="hero">
        <div>📊</div>
        <h1>MIDAS（片頭痛障害評価スコア）完全リファレンスガイド</h1>
        ...
        <div className="hero-tags">
          ...
          <span className="hero-tag">Disability</span>
      ...
      <div className="layout">
        {/* SIDEBAR */}
        <MigraineDisabilityAssessmentSidebar />
...
D -->|"二次性頭痛が判明"| E["原疾患の治療\\n専門科紹介"]
D -->|"異常なし"| F
B -->|"フラグなし"| F["ICHD-3 診断基準による\\n一次性頭痛の診断"]
F --> G["MIDAS 質問票の実施\\n（記入時間：約5分）\\n+ 補足質問 A・B"]
G --> MOH{"⚠️ MOH リスク評価\\n急性期薬の使用日数を確認\\nNSAIDs: ≥15日/月 ≥3ヵ月\\nトリプタン: ≥10日/月 ≥3ヵ月"}
MOH -->|"MOH 疑い"| MOH_TX["MOH 対応プロトコル\\n（薬剤漸減・解毒）"]
MOH -->|"MOH なし"| H["スコア計算\\nQ1 + Q2 + Q3 + Q4 + Q5"]
MOH_TX --> H
H --> I{"MIDAS スコア分類"}
I -->|"0〜5点\\nGrade I"| J["🟢 軽度/障害なし\\n急性期治療最適化\\n生活習慣介入\\n頭痛日誌開始"]
...
M --> O
O --> P{"12週後 MIDAS 再評価"}
P -->|"≥50% 改善\\nまたは MID 達成"| Q["✅ 治療継続・維持\\nMIDAS レスポンダーと判定"]
P -->|"改善不十分"| R["⚙️ 治療計画の見直し\\nMOH 再評価\\nCGRP mAb 変更・追加検討\\n多職種介入の強化"]
Q --> S["6ヵ月・1年後の長期フォロー"]
R --> S
...
style MOH_TX fill:#c0392b,color:#ffffff
style J fill:#2ecc71,color:#ffffff
style K fill:#f39c12,color:#ffffff
style L fill:#e67e22,color:#ffffff
...
SELECT --> TRAD["従来型予防薬 [Grade A]\\nβ遮断薬（プロプラノロール）\\nCa拮抗薬（フルナリジン）\\nTCA（アミトリプチリン）\\n抗てんかん薬（トピラマート）\\n*妊娠禁忌・特殊集団に注意"]
SELECT --> CGRP["CGRP 標的療法 [Grade A]\\nErenumab / Fremanezumab\\nGalcanezumab / Eptinezumab\\n*心血管安全性の優位性"]
SELECT --> BONTA["ボツリヌストキシン A [Grade A]\\n慢性片頭痛（ICHD-3 1.3）\\nのみ適応"]

style START fill:#2C3E50,color:#fff
style STRONG fill:#c0392b,color:#fff
style PREV fill:#e67e22,color:#fff
style ACUTE fill:#27ae60,color:#fff
...
P1 --> P2["🟡 回復初期（Week 2〜8）\\nMIDAS スコアの緩やかな改善が見込まれる\\n4週ごとに HIT-6 で変化をモニタリング\\nCBT：薬剤依存の認知パターンへの介入"]

P2 --> P3["🟢 安定期（Week 8〜）\\n全プログラムの段階的導入\\nベースライン比での MIDAS スコア改善を確認\\n予防療法の有効性を MIDAS + 頭痛日誌で評価"]

P3 --> P4["✅ 再発予防（長期）\\n頭痛日誌の永続的継続\\nMOH 再発の早期検出教育\\n月間薬剤使用日数と MIDAS スコアを定期モニタリング\\n6ヵ月毎の外来フォローアップ"]
