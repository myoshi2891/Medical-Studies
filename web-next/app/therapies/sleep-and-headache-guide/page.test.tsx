...
import SleepAndHeadacheGuidePage from "./page";
...
/** ソース HTML（Sleep-and-headache-guide.html）から実測した忠実転記の契約値。 */
...
const TABLE_COUNT = 7;
const NAV_COUNT = 10;
const EXTERNAL_LINK_COUNT = 48;
const HERO_H1 = "睡眠と頭痛 — エビデンスに基づく基礎知識と睡眠衛生ガイド";

describe("SleepAndHeadacheGuidePage: 契約（忠実転記）", () => {
  it("hero の <h1> がソースのページタイトルと一致する", () => {
    const { container } = render(<SleepAndHeadacheGuidePage />);
  ...
  it("<h3> の個数がソースと一致する（セクション内サブ見出し 13 個）", () => {
    const { container } = render(<SleepAndHeadacheGuidePage />);
 ...
 */
describe("SleepAndHeadacheGuidePage: 関連ページ導線", () => {
  const HREF = "/therapies/sleep-and-headache-guide";
