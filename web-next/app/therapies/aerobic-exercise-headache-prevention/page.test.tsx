...
import AerobicExerciseHeadachePreventionPage from "./page";
...
/** ソース HTML（Aerobic-exercise-headache-prevention.html）から実測した忠実転記の契約値。 */
...
const NAV_COUNT = 10;
const EXTERNAL_LINK_COUNT = 16;
const HERO_H1 = "頭痛予防のための有酸素運動 ― エビデンスに基づくステップガイド";

describe("AerobicExerciseHeadachePreventionPage: 契約（忠実転記）", () => {
  it("hero の <h1> がソースのページタイトルと一致する", () => {
    const { container } = render(<AerobicExerciseHeadachePreventionPage />);
    const hero = container.querySelector(".hero h1");
    expect(hero?.textContent).toBe(HERO_H1);
  });

  it("section.sec の id 配列が s1..s10 と一致する", () => {
    const { container } = render(<AerobicExerciseHeadachePreventionPage />);
    const ids = Array.from(container.querySelectorAll("section.sec")).map((s) => s.id);
    expect(ids).toEqual(SECTION_IDS);
  });

  it("<h2> の個数がソースと一致する（各セクションのタイトル 10 個）", () => {
    const { container } = render(<AerobicExerciseHeadachePreventionPage />);
  ...
  it("<h3> の個数がソースと一致する（セクション内サブ見出し 5 個）", () => {
    const { container } = render(<AerobicExerciseHeadachePreventionPage />);
 ...
 */
describe("AerobicExerciseHeadachePreventionPage: 関連ページ導線", () => {
  const HREF = "/therapies/aerobic-exercise-headache-prevention";
